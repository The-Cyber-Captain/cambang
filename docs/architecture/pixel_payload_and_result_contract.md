...
### 7.7 Godot render-resource thread affinity

Godot render resources and render-resource-owning adapter objects have
thread-affine destruction requirements distinct from ordinary C++ ownership.

This includes, where applicable:

- `RenderingServer` texture RIDs;
- `RenderingDevice` RIDs;
- `Texture2D`, `Texture2DRD`, and similar Godot wrappers whose destruction may
  release render resources;
- adapter state whose destructor, reset, replacement, or final-owner release can
  trigger any of the above.

The following rules apply to CPU-backed and GPU-backed display paths alike.

1. Arbitrary-thread ownership release must not directly call
   `RenderingServer::free_rid()`, `RenderingDevice::free_rid()`, or an equivalent
   render-resource operation.
2. Arbitrary-thread ownership release must not destroy the final Godot wrapper
   reference when that destruction can release a render resource.
3. A destructor reached from arbitrary ownership may only perform a non-throwing
   handoff to the approved Godot/render-resource release context, or relinquish
   a thread-neutral ownership record whose eventual destruction is confined to
   that context.
4. The actual RID free and final render-wrapper destruction occur only in the
   approved drain context.
5. Registry, cache, and release-queue locks must be released before RID freeing
   or final Godot-wrapper destruction occurs.
6. Destructor-originating handoff and cleanup paths must not throw. Allocation
   failure, queue saturation, late teardown, or an unavailable drain must never
   fall back to current-thread render-resource destruction.
7. If safe release can no longer be scheduled during terminal teardown, the
   resource is retained in an explicit non-destructing terminal quarantine with
   truthful diagnostics rather than destroyed on an unsafe thread.
8. Teardown ordering must stop new render-resource creation, close new release
   admission as appropriate, drain accepted releases on the approved context,
   and only then uninstall or invalidate the release service.
9. Clearing or abandoning a pending-release container is not equivalent to
   safely draining it. Pending Godot wrapper references and RIDs must not be
   destroyed merely because the queue or bridge is being torn down.
10. Direct release is permitted only when the caller is already executing in
    the approved render-resource context and no broader registry/release lock is
    held.
11. Arbitrary worker threads may enqueue credited ownership only. Drain-helper
    creation and drain scheduling occur from the Godot thread during service
    installation, process-frame handling, or controlled extension teardown.
12. Teardown draining proceeds to ownership quiescence, including release chains
    where destroying a wrapper releases retained state that enqueues another
    RID. The global release-credit bound is also the progress bound; unavailable
    render contexts terminate the drain pass rather than causing an unbounded
    retry loop.
13. A transient scheduling failure retains queued ownership and is retried from
    a later approved Godot scheduling point. Unrecoverable admission or credit
    failure instead latches one internal failure, emits one production
    diagnostic, stops the Godot runtime boundary, and prevents restart while the
    release service remains failed.
14. Terminal teardown linearizes its final admission cutoff with arbitrary-thread
    handoffs. A handoff admitted before that cutoff is observed and drained to
    recursive quiescence; a later handoff cannot enter pending or emergency
    storage and instead follows bounded terminal quarantine.
15. Persistent live-display demand is owned by an internal exact lease token tied
    to the current retained stream incarnation. Result-store clear or stream
    removal invalidates prior tokens, and releasing a stale token cannot decrement
    demand for a later stream incarnation that reuses the same authored ID.

Display adapters may use shared ownership for public/live-view lifetime, but
final shared-owner destruction must obey these rules. `std::shared_ptr` or Godot
`Ref` lifetime alone does not establish a safe destruction thread.

This contract is internal. It does not change the Godot public API, result
shape, Backing Plan, Backing State, Operation Support, or Access Evidence.
---
