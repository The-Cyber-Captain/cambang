...
* Synthetic maintainer output-form selection is exposed through the project setting `cambang/maintainer/synthetic_producer_output_form=runtime_default|cpu_only|cpu_gpu|gpu_only`; host command-line runs may pass `--cambang-synth-producer-output-form=...`, which updates that same project setting before provider construction; there is no environment-variable fallback for this selection;
