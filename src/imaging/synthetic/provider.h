...
public:
 ...
 public:
  ...
  // flush_strand=true (the default, and the only valid choice for host-stepped
  // advance_timeline() and harness/smoke callers) blocks until the strand
  // worker has delivered every provider->core callback generated by this
  // advance, so publish-only checks never race queued frame delivery.
  // flush_strand=false is for the free-running per-frame runtime tick only:
  // Godot observes state exclusively through tick-bounded snapshot
  // publication, which happens when facts integrate on the core thread
  // regardless of when the strand delivers them, so that caller does not need
  // to stall on strand drain every frame.
  void advance(uint64_t dt_ns,
               bool allow_paused_timeline_step = false,
               bool flush_strand = true);
