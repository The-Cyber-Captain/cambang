...
int frame_starvation(VerifyCaseProviderKind provider_kind) {
  ...
  if (!h.wait_for_stream_quiescence(VerifyCaseHarness::kStreamId, error)) {
    cli::error("FAIL: ", error);
    return 1;
  }
  h.tick();
  ...
  const auto* quiesced = VerifyCaseHarness::find_stream(*h.observed().raw, VerifyCaseHarness::kStreamId);
  if (!quiesced || quiesced->mode != CBStreamMode::FLOWING) {
    fail_step(2, "stream not flowing at starvation boundary");
    return 1;
  }
  const uint64_t before_frames = quiesced->frames_received;
  if (!h.request_publish_only(error)) {
    cli::error("FAIL: ", error);
    return 1;
  }
