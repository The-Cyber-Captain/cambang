...
for ($iteration = 1; $iteration -le $Iterations; $iteration++) {
    $label = "cpu_teardown_{0:D4}" -f $iteration
    & $launcher `
        -GodotExe $GodotExe `
        -ProjectPath $projectFullPath `
        -Script "res://scripts/cpu_display_teardown_race_stress.gd" `
        -CaptureLogs `
        -LogRoot $logRootResolved `
        -RunLabel $label `
        -TimeoutSec $TimeoutSec `
        -HardFailurePatterns $hardFailures `
        -ExtraArgs @("--cambang-synth-producer-output-form=cpu_only")
...
Write-Host ("CPU teardown stress: iterations={0} passed={1} failed={2} artifacts={3}" -f `
