...
- **State `use_mingw` explicitly.** `auto` resolves from the *shell*, so the same command selects MinGW under Git Bash and MSVC under PowerShell. A MinGW Windows GDE build still succeeds — it just silently produces a **synthetic-only** plugin (~63 MB, vs ~8 MB for MSVC), announced only by `gde_provider_status=not_compiled` in the config banner. Verify that banner before trusting a platform-backed run.
